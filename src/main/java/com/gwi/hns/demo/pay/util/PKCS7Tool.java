...
public class PKCS7Tool {
     ...
     * @throws IOException
     */
    public static PKCS7Tool getSigner(InputStream keyStoreStream, String type, String keyStorePassword,
    ...
    public static PKCS7Tool getVerifier(InputStream rootCertStream, String type)
        ...
        if ("DER".equals(type)) {
            try {
                rootCertificates = new Certificate[] {
                        certificatefactory.generateCertificate(rootCertStream) };
            } catch (Exception exception) {
                if (debug)
                    exception.printStackTrace();
                InputStream is = new ByteArrayInputStream(new BASE64Decoder().decodeBuffer(rootCertStream));
    ...
    public String sign(byte[] data) throws Exception {
        ...
        contentInfo = contentInfoConstructor.newInstance(new Object[] { data_oid, null });
        // 根证书
        ...
        Constructor<SignerInfo> signerInfoConstructor = SignerInfo.class
                .getConstructor(new Class[] { x500Name, BigInteger.class, algorithmId, PKCS9Attributes.class,
                        algorithmId, byte[].class, PKCS9Attributes.class });
        // 签名信息
        SignerInfo si = signerInfoConstructor.newInstance(new Object[] { x500NameObject, // X500Name,
                                                                                         // issuerName,
        ...
        // PKCS7
        Constructor<PKCS7> pkcs7Constructor = PKCS7.class
                .getConstructor(new Class[] { digestAlgorithmIds.getClass(), ContentInfo.class,
                        X509Certificate[].class, signerInfos.getClass() });
        PKCS7 p7 = pkcs7Constructor
        ...
        for (int i = 0; i < sis.length; i++) {
            ...
            if (i == 0 && dn != null) {
                ...
                if (!dn.equals(name.getName(X500Principal.RFC1779)) && !new X500Principal(dn).equals(name))
                    throw new SignatureException("Signer dn '" + name.getName(X500Principal.RFC1779)
                            + "' does not matchs '" + dn + "'");
            }
        }
    }
